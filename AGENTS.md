...
- **Tooling setup checks.** First admin bootstrap redirects to `/ui/setup/tooling/`; `/backends/status` reports daemon, runner image, `gh`, GitHub MCP, Claude, Codex, and model-discovery health so users can verify credentials before configuring the fleet.
