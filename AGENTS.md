...
- **Prompt**, a reusable catalog asset that may be global, workspace-scoped, or repo-scoped. Import/export and API surfaces expose a stable public ref as `id`; the database stores an opaque internal primary key for FK integrity. Agents use `prompt_id`, or the human selector `prompt_ref` plus optional `prompt_scope` (`global`, `workspace`, or `workspace/owner/repo`, case-insensitive), and the daemon resolves that selector to the internal prompt key. Inline agent prompt bodies are unsupported; create or select a prompt catalog entry first.
...
- **Skill**, a reusable chunk of guidance referenced by stable public ref in agents. Display names may repeat across global, workspace, and repo scopes; skill text is concatenated before the selected catalog prompt at render time. The database uses opaque internal IDs behind those refs.
- **Catalog delegation**, optional GitHub ownership for reusable prompts, skills, and operator-defined guardrails. SQLite remains the runtime mirror; when delegation is enabled, service-layer catalog mutation paths return a delegated/read-only conflict and daemon-owned workflow configuration remains writable.
...
- **Service**, the mutable use-case layer. REST and MCP handlers decode wire shapes, call `internal/service`, and map typed errors; service owns transaction orchestration and business/domain validation. `internal/store` exposes SQLite execution, migrations, load helpers, tx-aware persistence primitives, and DB-level safety checks such as FK-friendly existence checks and conflict detection.
...
- **Config is loaded from SQLite at startup.** Seed an empty SQLite store via `POST /import`; YAML is import/export only, not a runtime input. Manage subsequent changes via the CRUD API or the web dashboard. Prompt and skill content is stored in the database; changes via the API or UI take effect on the next agent run without a restart. Fleet-only imports that omit `runtime:` preserve the existing runner image, container constraints, and internal analyst backend/model.
- **Backend discovery lifecycle.** Startup auto-discovery runs only when the backends table is empty. Manual refresh is explicit via `POST /backends/discover`; `GET /backends/status` is diagnostics-only. Backend health means CLI detection plus auth/model-readiness, not just binary presence.
- **Runtime toolchain.** The `agents` Docker image is the control plane and stays minimal. The `agents-runner` image contains Claude Code, Codex, `gh`, git, Go, Rust/Cargo, Node/npm, TypeScript, Flutter/Dart, and runtime tools. The daemon starts one fresh runner container per run through the Docker Engine API and injects allowlisted env credentials (`GITHUB_TOKEN`, Claude vars, `CODEX_AUTH_JSON_BASE64`, `OPENAI_API_KEY`) from its own environment. Runner setup also configures git identity from `AGENTS_GIT_USER_NAME` / `AGENTS_GIT_USER_EMAIL` when set so agents do not invent commit authors. Codex auth cache is materialized from `CODEX_AUTH_JSON_BASE64` inside the ephemeral runner container; no host `~/.codex` volume is mounted. If both Codex env auth modes are set, `CODEX_AUTH_JSON_BASE64` wins over `OPENAI_API_KEY`; refreshed Codex auth state is not persisted out of the ephemeral runner.
