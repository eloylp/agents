...
internal/
  ...
  service/                      # mutable fleet/config use cases: validation orchestration and transaction ownership
...
- **Service**, the mutable use-case layer. REST and MCP handlers decode wire shapes, call `internal/service`, and map typed errors; `internal/store` exposes SQLite execution, migrations, load helpers, and tx-aware persistence primitives.
