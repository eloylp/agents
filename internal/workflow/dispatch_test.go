...
// the dispatch slot must not be visible so the autonomous scheduler can
...
// a phantom committed slot could remain, causing both paths to skip.
...
// test for the cron-first dedup ordering: when an autonomous run claims the
// slot via TryMarkAutonomousRun, dispatches targeting the same agent/repo
// with number=0 (autonomous context) must be suppressed for the full
// dedup_window_seconds, both while the run is in-flight and after it completes.
func TestMarkAutonomousRunSuppressesNearSimultaneousDispatch(t *testing.T) {
	t.Parallel()
	q := &fakeQueue{}
	d := testDispatcher(t, q)

	// Simulate: cron run atomically checks for dispatch claims and writes the cron mark.
...
func TestMarkAutonomousRunDoesNotSuppressDispatchForDifferentNumber(t *testing.T) {
	...
	// Cron run for coder marks (coder, owner/repo, 0).
	if !d.TryMarkAutonomousRun("coder", "owner/repo", time.Now()) {
		t.Fatal("TryMarkAutonomousRun: expected true on first call (no prior dispatch)")
	}
...
func TestPostRunDispatchSuppressedWithinDedupWindow(t *testing.T) {
	...
	// Autonomous run claims the slot and writes the cron mark.
	now := time.Now()
