...
// TestBuildDeliveryRespectsTotalBudget verifies that both Claude and non-Claude
// backends truncate at the same logical boundary.  The reviewer asked for a
// regression test asserting that the same request produces equivalent content
// across backends (PR #149 review, second round).
func TestBuildDeliveryRespectsTotalBudget(t *testing.T) {
	...
	tests := []struct {
		...
		user         string
		maxChars     int
		// expected codex (non-claude) stdin — same logical combined string
		wantCodexStdin string
		// expected claude --append-system-prompt value
		wantClaudeSystem string
		// expected claude stdin (user portion after re-split)
		wantClaudeStdin string
	}{
		{
			// Reviewer's exact example: System=5, sep=2, budget=8 → User gets 1 char.
			name:             "user truncated to remaining headroom",
			system:           "abcde",
			user:             "0123456789",
			maxChars:         8,
			wantCodexStdin:   "abcde\n\n0",
			wantClaudeSystem: "abcde",
			wantClaudeStdin:  "0",
		},
		{
			// Budget exactly equals system length: truncation cuts into separator,
			// so no user content survives on either backend.
			name:             "system fills budget exactly",
			system:           "abcde",
			user:             "xyz",
			maxChars:         5,
			wantCodexStdin:   "abcde",
		...
		{
			// Budget is smaller than system: truncated system is sent on both paths.
			name:             "budget smaller than system",
			system:           "abcde",
			user:             "xyz",
			maxChars:         3,
			wantCodexStdin:   "abc",
			wantClaudeSystem: "abc",
			wantClaudeStdin:  "",
		},
		{
			// Unlimited budget (maxChars=0): full content passes through.
			name:             "unlimited budget",
			system:           "sys",
			user:             "usr",
			maxChars:         0,
			wantCodexStdin:   "sys\n\nusr",
			wantClaudeSystem: "sys",
			wantClaudeStdin:  "usr",
		},
		{
			// No system content: behaves identically to the old single-prompt path.
			name:             "empty system falls back to user-only stdin on both backends",
			system:           "",
			user:             "hello",
			maxChars:         3,
			wantCodexStdin:   "hel",
			wantClaudeSystem: "", // no --append-system-prompt flag
			wantClaudeStdin:  "hel",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			codexRunner := &CommandRunner{
				backendName:    "codex",
				args:           baseArgs,
				maxPromptChars: tt.maxChars,
			}
			claudeRunner := &CommandRunner{
				backendName:    "claude",
				args:           baseArgs,
				maxPromptChars: tt.maxChars,
			}
			req := Request{System: tt.system, User: tt.user}

			codexArgs, codexStdin := codexRunner.buildDelivery(req)
			claudeArgs, claudeStdin := claudeRunner.buildDelivery(req)

			// Codex: args unchanged, full combined truncated prompt on stdin.
			if codexStdin != tt.wantCodexStdin {
				t.Errorf("codex stdin: got %q, want %q", codexStdin, tt.wantCodexStdin)
			}
			if !slices.Equal(codexArgs, baseArgs) {
				t.Errorf("codex args: got %v, want %v", codexArgs, baseArgs)
			}

			// Claude with system content: --append-system-prompt flag + user stdin.
			if tt.system != "" {
				idx := slices.Index(claudeArgs, "--append-system-prompt")
				if idx < 0 || idx+1 >= len(claudeArgs) {
					t.Fatalf("claude args missing --append-system-prompt: %v", claudeArgs)
				}
				gotSystem := claudeArgs[idx+1]
				if gotSystem != tt.wantClaudeSystem {
					t.Errorf("claude --append-system-prompt: got %q, want %q", gotSystem, tt.wantClaudeSystem)
				}
			} else {
				// No system → no --append-system-prompt, falls through to codex path.
				if slices.Contains(claudeArgs, "--append-system-prompt") {
					t.Errorf("claude args should not contain --append-system-prompt when system is empty: %v", claudeArgs)
				}
			}
			if claudeStdin != tt.wantClaudeStdin {
				t.Errorf("claude stdin: got %q, want %q", claudeStdin, tt.wantClaudeStdin)
			}

			// Key invariant: the logical combined delivered content must be the same
			// on both backends so the truncation boundary is identical.
			codexLogical := codexStdin
			claudeLogical := combineSystemUser(claudeArgs[len(claudeArgs)-1], claudeStdin)
			if tt.system == "" {
				claudeLogical = claudeStdin
			}
			// Both logical forms must have the same rune count (= same budget consumed).
			if utf8.RuneCountInString(codexLogical) != utf8.RuneCountInString(claudeLogical) {
				t.Errorf("truncation boundary mismatch: codex logical %q (%d runes) != claude logical %q (%d runes)",
					codexLogical, utf8.RuneCountInString(codexLogical),
					claudeLogical, utf8.RuneCountInString(claudeLogical))
			}
		})
	}
}
