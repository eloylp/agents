...
func TestPromptForAgentRejectsUnresolvablePromptRefs(t *testing.T) {
	t.Parallel()
		...
		},
		{
			...
			cfg: Config{
				Prompts: []fleet.Prompt{
					...
					{WorkspaceID: fleet.DefaultWorkspaceID, Name: "shared", Content: "Workspace prompt."},
				},
			},
			agent: fleet.Agent{
				Name:        "coder",
	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := tc.cfg.PromptForAgent(tc.agent, tc.repo)
			if err == nil {
				t.Fatalf("PromptForAgent succeeded, want error containing %q", tc.wantErr)
			}
