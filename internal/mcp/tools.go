...
func registerTools(srv *server.MCPServer, deps Deps) {
	...
	srv.AddTool(
		mcpgo.NewTool("get_prompt",
			mcpgo.WithDescription("Fetch one prompt catalog entry by stable id, or by name plus optional scope path (global, workspace, workspace/owner/repo)."),
			mcpgo.WithString("id",
				mcpgo.Description("Stable prompt id. Preferred for scripts and required when name/scope is ambiguous."),
			),
			mcpgo.WithString("name",
				mcpgo.Description("Prompt display name. If id is omitted, resolves with optional scope, or workspace_id/repo."),
			),
			mcpgo.WithString("scope",
				mcpgo.Description("Optional case-insensitive catalog scope path: global, workspace, or workspace/owner/repo."),
			),
	...
	srv.AddTool(
		mcpgo.NewTool("list_workspace_guardrails",
			mcpgo.WithDescription("List the guardrail catalog references selected by one workspace. Defaults to Default when workspace is omitted."),
			mcpgo.WithString("workspace",
	...
	if deps.Observe != nil {
		...
		srv.AddTool(
			mcpgo.NewTool("list_improvement_feedback",
				mcpgo.WithDescription("List stored /agents improve feedback events. These are raw evidence records used by self-improvement proposal analysis. Omitted workspace lists the global improvement log."),
				mcpgo.WithString("workspace",
					mcpgo.Description("Optional workspace id or display name to narrow the global improvement log."),
				),
		...
		srv.AddTool(
			mcpgo.NewTool("list_improvement_recommendations",
				mcpgo.WithDescription("List durable self-improvement proposal records. Proposal records are review-only until their bundle is published. Omitted workspace lists the global proposal set."),
				mcpgo.WithString("workspace",
		...
		srv.AddTool(
			mcpgo.NewTool("get_improvement_recommendation",
				mcpgo.WithDescription("Fetch one self-improvement proposal record with its linked feedback evidence."),
				mcpgo.WithString("id",
					mcpgo.Required(),
					mcpgo.Description("Proposal record id."),
				),
			),
		...
		srv.AddTool(
			mcpgo.NewTool("update_improvement_recommendation_status",
				mcpgo.WithDescription("Reject one self-improvement proposal record. Terminal decisions never publish or apply catalog changes."),
				mcpgo.WithString("id",
					mcpgo.Required(),
					mcpgo.Description("Proposal record id."),
				),
				mcpgo.WithString("status",
					mcpgo.Required(),
					mcpgo.Description("New terminal decision. Only rejected is supported."),
				),
				mcpgo.WithString("reason",
					mcpgo.Description("Optional human decision reason to store with the rejected proposal."),
				),
			),
		...
		srv.AddTool(
			mcpgo.NewTool("clarify_improvement_recommendation",
				mcpgo.WithDescription("Upsert the editable maintainer clarification for one proposal record and enqueue a fresh self-improvement analysis run. Failed clarification runs can be retried by sending the same stored body."),
				mcpgo.WithString("id",
					mcpgo.Required(),
					mcpgo.Description("Proposal record id."),
				),
				mcpgo.WithString("body",
					mcpgo.Required(),
					mcpgo.Description("Latest clarification text. This replaces the previous clarification for the proposal record."),
				),
		...
		srv.AddTool(
			mcpgo.NewTool("link_improvement_proposal_bundle_item",
				mcpgo.WithDescription("Resolve a create-new proposal bundle item as already covered by an existing catalog asset. This does not attach the asset to any agent."),
				mcpgo.WithString("bundle_id", mcpgo.Required(), mcpgo.Description("Proposal bundle id.")),
				mcpgo.WithString("item_id", mcpgo.Required(), mcpgo.Description("Proposal bundle item id.")),
	...
	if deps.Fleet != nil {
		...
		srv.AddTool(
			mcpgo.NewTool("update_skill",
				...
				mcpgo.WithString("state",
					mcpgo.Description("When publish is false, create this version as draft or proposal. Defaults to draft."),
				),
				mcpgo.WithString("source_type",
					mcpgo.Description("When publish is false, record the version source: manual, feedback_recommendation, or audit_recommendation."),
				),
				mcpgo.WithString("source_ref",
					mcpgo.Description("When publish is false, optional source identifier such as a recommendation id."),
				),
				mcpgo.WithString("author",
					mcpgo.Description("When publish is false, optional actor recorded on the version metadata."),
				),
				mcpgo.WithString("changelog",
					mcpgo.Description("When publish is false, optional rationale recorded on the version metadata."),
				),
			),
		...
		srv.AddTool(
			mcpgo.NewTool("create_prompt",
				mcpgo.WithDescription("Create or update a prompt catalog entry. Use scope (global, workspace, workspace/owner/repo) or workspace_id/repo to set visibility."),
				mcpgo.WithString("name",
					mcpgo.Required(),
				...
				mcpgo.WithString("scope",
					mcpgo.Description("Optional case-insensitive catalog scope path: global, workspace, or workspace/owner/repo. Takes precedence over workspace_id/repo."),
				),
				mcpgo.WithString("repo",
		...
		srv.AddTool(
			mcpgo.NewTool("delete_prompt",
				mcpgo.WithDescription("Delete a prompt by stable id, or by name plus optional scope path. Fails while any agent references it. Same path as DELETE /prompts/{id}."),
				mcpgo.WithString("id",
					mcpgo.Description("Stable prompt id. Preferred for scripts and required when name/scope is ambiguous."),
				),
				mcpgo.WithString("name",
		...
		srv.AddTool(
			mcpgo.NewTool("create_agent",
				...
				mcpgo.WithArray("skills",
					mcpgo.Description("Optional list of skill names to compose into the agent's system prompt. Agents always track each skill's latest published catalog version."),
					mcpgo.Items(map[string]any{"type": "string"}),
				),
		...
		srv.AddTool(
			mcpgo.NewTool("update_agent",
				...
				mcpgo.WithString("prompt_ref",
					mcpgo.Description("Prompt name. Omit to leave unchanged. Pair with prompt_scope when names are ambiguous."),
				),
				mcpgo.WithString("prompt_scope",
					mcpgo.Description("Optional case-insensitive prompt scope path: global, workspace, or workspace/owner/repo."),
				),
				mcpgo.WithString("prompt_id",
				...
				mcpgo.WithArray("skills",
					mcpgo.Description("List of skill names. Agents always track each skill's latest published catalog version. Omit to leave unchanged; pass an empty array to clear."),
					mcpgo.Items(map[string]any{"type": "string"}),
				),
				mcpgo.WithArray("can_dispatch",
