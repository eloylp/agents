...
func registerRunnersTools(srv *server.MCPServer, deps Deps) {
	...
	srv.AddTool(
		mcpgo.NewTool("list_runners",
			mcpgo.WithDescription("List runner rows: each row is one event in the durable queue, expanded into per-agent rows once traces have been recorded for it. While an event is in-flight (no traces yet) one row appears with agent=null and status=enqueued|running. Once completed, one row per fanned-out agent appears with status=success|error. Same path as GET /runners."),
