...
Sensitive endpoints require daemon auth after the first local user is created. Browser clients use the `agents_session` `HttpOnly` cookie; REST and MCP clients send a DB-backed API token with `Authorization: Bearer <token>`. `/status`, `/webhooks/github`, `/auth/status`, `/auth/login`, `/auth/bootstrap`, UI static assets, and the local-model proxy remain public where applicable.
...
| `GET` | `/traces/{span_id}/stream` | Server-Sent Events stream of normalized `ai.StreamEvent` payloads (one JSON object per `data:` line) for an in-flight (or recently-finished) span. The daemon parses each AI CLI stdout line through a per-backend stream parser (`internal/ai/stream_events.go`) and emits the canonical shape `{kind: tool_use\|tool_result\|thinking\|usage\|raw, tool, server, input, output, text, error, duration_ms, usage}`, so clients render one shape instead of three CLI-specific ones. Replays the per-span ring buffer first, then live-tails until the run ends or the client disconnects. Emits a final `event: end` SSE message when the run finishes. `404` when no stream exists for the span (never registered, or grace window elapsed). In-memory only, daemon restart loses the live data; the trace row in SQLite keeps the structured record. |
