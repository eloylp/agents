...
- **`--dangerously-skip-permissions`.** Required for headless Claude operation. The recommendation is to run the daemon inside its Docker container — the container *is* the project's sandbox, and skipping it means the AI CLIs run with operator privileges directly on the host. See [`quickstart.md`](quickstart.md) for why Docker Compose is the strongly recommended deployment path.
...
4. **Capability isolation.** The Docker container is the project's sandbox: it isolates the runners' filesystem and process tree from the host, bounds blast radius, and prevents pivot to other host services. **The deployment recommendation is unambiguous — run the daemon in its container, never the binary directly on the host.** Running on the host means the AI CLIs (`claude`, `codex`) execute with operator privileges and the sandbox-bypass flags (`--dangerously-skip-permissions`, `--dangerously-bypass-approvals-and-sandbox`) bind to the operator's own user — at that point the project provides no isolation at all. Inside the container, two gaps remain that the project does not yet close:
   - **Concurrent runs share the container's filesystem** — a compromised agent can read another in-flight agent's working data. Per-deployment policy (e.g. running one agent per container) is the workaround until the project ships invocation-level isolation.
   - **The container today bind-mounts `~/.claude.json` and `~/.codex/` from the host** so the AI CLIs reuse host auth. A compromised runner can exfiltrate these tokens — the host pivots back through the bind-mount even though the container itself is isolated. Closing this gap is the next architectural step on the project's roadmap: replace the bind-mounts with a per-container `claude login` flow persisted to a named volume, so the container has its own auth and there is no host-token leak channel. Until that lands, treat the host's Claude/Codex auth as compromised the moment any agent run is.
