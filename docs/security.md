...
- **`/queue` is operator-grade.** The endpoint exposes payload metadata (kinds, repos, numbers) and lets a caller delete or retry queued events. Gate it behind the same auth proxy as `/ui/` and the rest of the management surface.
