...
- **Operator-grade endpoints behind auth.** `/runners`, `/traces`, `/traces/{span_id}/prompt`, `/ui/` and the rest of the management surface must sit behind your auth proxy. The composed prompt every run sees is persisted on the trace span (gzipped) so operators can inspect what the agent saw — that data is gated by your proxy, not by the daemon.
