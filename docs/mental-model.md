...
4. **The selected prompt.** The reusable prompt catalog content selected by stable `prompt_id`, or by resolving `prompt_ref` plus optional case-insensitive `prompt_scope` (`global`, `workspace`, or `workspace/owner/repo`).
