...
5. **Available experts roster.** When the agent has valid dispatch targets in `can_dispatch:`, the daemon injects an `## Available experts` section listing targets that exist, have `allow_dispatch: true`, and have a `description`.
