...
- HTTP endpoints:
  ...
  - `POST /mcp` — MCP (Model Context Protocol) Streamable HTTP endpoint. Registered MCP clients (Claude Code, Cursor, Cline) discover fleet-management tools automatically. First-cut tools: `list_agents`, `get_agent`, `list_skills`, `get_skill`, `list_backends`, `get_backend`, `list_repos`, `get_repo`, `get_status`, `trigger_agent`. Additional CRUD write, observability, and config import/export tools are tracked in follow-ups to #227.
