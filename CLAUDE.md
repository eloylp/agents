...
- HTTP endpoints:
  ...
  - `GET /export`, `POST /import`, export/import fleet config as YAML (includes agents, skills, repos, guardrails, token_budgets, and daemon.ai_backends).
...
- **Token budgets**: per-scope (global, backend, or agent) token caps over daily/weekly/monthly periods. Checked before each agent run via `BudgetChecker` in the engine; fail-open so a broken `token_budgets` table never blocks runs. Alert thresholds (0 = disabled, 1-100 = percentage of cap) drive the NavBar danger banner. Token leaderboard aggregates per-agent usage from the `traces` table. Budget CRUD is exposed at `/token_budgets` (REST) and via the `list_token_budgets`, `create_token_budget`, `update_token_budget`, `delete_token_budget` MCP tools. The `get_token_leaderboard` MCP tool and `GET /token_leaderboard` endpoint aggregate per-agent usage. Budgets are included in `/export` and `/import` round-trips.
