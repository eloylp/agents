...
The import/export config file is a fleet strategy document with global catalog domains plus workspace-local wiring:
...
An agent is a workspace-local capability definition, it doesn't run until a repo in the same workspace binds it. Agents reference global prompts through `prompt_ref`; legacy inline prompt imports are accepted into the prompt catalog for migration compatibility. A binding sets exactly one trigger: `labels: [...]`, `events: [...]`, or `cron: "..."`. The same agent can have multiple bindings on the same repo with different triggers.
...
- HTTP endpoints:
  ...
  - `GET /export`, `POST /import`, export/import fleet config as YAML (includes global prompts, skills, guardrails, backends, and workspace-local agents, repos, guardrail references, and token budgets).
...
- **Prompt guardrails**: the global `guardrails` table holds reusable policy blocks, and `workspace_guardrails` selects the ordered subset rendered for each workspace. The renderer combines mandatory dynamic workspace/repository boundary guidance and selected workspace guardrails in one guardrails section ahead of skills and global prompt content. Operators can edit the global catalog and manage selected workspace references via the `Guardrails` tab in `/ui/config`, the REST surface, or the MCP tools.
